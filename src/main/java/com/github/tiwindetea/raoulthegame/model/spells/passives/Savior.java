...
public class Savior extends Spell<LivingThing> {
    ...
    private static final double BASE_HP_REGEN = 20;
    private static final double PERCENTAGE_HP_REGEN = 90;
    ...
    public double ownerDamaged(@Nullable LivingThing source, double damages) {
        if (this.cooldown <= 0) {
            ...
            if (owner != null) {
                if (owner.getHitPoints() - damages < owner.getMaxHitPoints() * TRIGGER_TRESHOLD_PERCENTAGE / 100
                        || owner.getHitPoints() - damages < TRIGGER_TRESHOLD_HEALTH) {
                    this.cooldown = this.baseCooldown;
                    owner.damage(-(owner.getMaxHitPoints() * PERCENTAGE_HP_REGEN / 100 + BASE_HP_REGEN + 1), null);
                    if (this.pid != -1) {
